...
//         addLog(
//           <>
//             <i className="fa-solid fa-check"></i> Wiadomość wysłana
//           </>,
//           "success"
//         );
//         setMessage("");
...
import React, { useState } from "react";
import { FiSend, FiCheck, FiAlertCircle, FiZap } from "react-icons/fi";
import toast from "react-hot-toast";
...
function QuickMessage({ gatewayIp, gatewayPort, addLog }) {
  ...
  const sendQuickSms = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addLog(
          <>
            <FiCheck style={{ color: "green" }} /> Wiadomość wysłana
          </>,
          "success"
        );
        setMessage("");
      ...
      addLog(
        <>
          <FiAlertCircle style={{ color: "red" }} /> Błąd: {error.message}
        </>,
        "error"
      );
    } finally {
  ...
  return (
    <div className="card">
      <h3>
        <FiZap style={{ marginRight: "0.5rem" }} /> Szybka wysyłka
      </h3>
